...
public class MonitorRepository extends BaseRepository
{
    ...
    public CompletableFuture<List<Monitor>> getAll() {
        ...
        ResponseEntity<List<Monitor>> res = template.exchange(baseUri + GET_ALL, GET, getEntity(), new ParameterizedTypeReference<List<Monitor>>() {});
        return CompletableFuture.completedFuture(res.getBody());
    }

    public CompletableFuture<Monitor> getById(long id) {

        ResponseEntity<Monitor> monitor = template.exchange(baseUri + GET_BY_ID + id, GET, getEntity(), new ParameterizedTypeReference<Monitor>() {});
